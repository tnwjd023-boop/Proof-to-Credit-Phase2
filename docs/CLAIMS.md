...
| Two distinct Sepolia sources can be proven into one CC3 aggregate snapshot and reserve bounded capacity | VERIFIED (Phase 2 public run) | `runs/phase2-20260913-multisource-01/manifest.json` and `audit.json`: A/B source events and epoch 1/2 checkpoints, actual BlockProver proof controls, totalDebt 50 then 40, reservation 20, and read-only negative controls. |
| Phase 2 demonstrates independent institutions, multiple source chains, real settlement, or asset transfers | FALSE / OUT OF SCOPE | The public run uses two distinct demo debts on Sepolia and one testnet signer. Settlement and institutional controls remain local-only. |
